...
export default class FullScreenContainer extends React.Component {
            ...
            <View key={'row_' + mediaList.indexOf(item)} style={styles.flex}>
              <TouchableWithoutFeedback
                ...
                <VideoPlayer
                  ...
                  displaySelectionButtons={displaySelectionButtons}
                  displayPlayButton={this.state.controlsDisplayed}
                  selected={item.selected}
                  onSelection={(isSelected) => {
                    onMediaSelection(mediaList.indexOf(item), isSelected);
                  }}
  ...
  _renderPlayButton() {
    const { controlsDisplayed, videoIsPlaying } = this.state;

    if(controlsDisplayed) {
      if(!videoIsPlaying) {
        return(
        <TouchableWithoutFeedback style={styles.playButtonIcon} onPress={this._onPlayButtonTapped}>
          <FontAwesome name="play-circle" size={50} color="white" />
        <TouchableWithoutFeedback>);
      } else if(videoIsPlaying) {
        return(
        <TouchableWithoutFeedback style={styles.playButtonIcon} onPress={this._onPlayButtonTapped}>
          <FontAwesome name="pause-circle" size={50} color="white" />
        <TouchableWithoutFeedback>);
      }
      return null;
    }
