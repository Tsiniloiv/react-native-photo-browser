...
export default class FullScreenContainer extends React.Component {
  ...
  constructor(props, context) {
    ...
    this._toggleComingSoon = this._toggleComingSoon.bind(this);
    this._renderPlayButton = this._renderPlayButton.bind(this);
    this._on3DButtonTapped = this._on3DButtonTapped.bind(this);
    ...
    if (!this.scrollView && !this.swiperView) {
      ...
      if (Platform.OS === 'ios'){
        if (this.state.currentIndex != index) {
            this.swiperView.scrollView.scrollTo({x: index, y:index, animated:false});
        }
  ...
  _on3DButtonTapped() {
    this._toggleControls();
    this.setState({displayComingSoon: !this.state.displayComingSoon});
  }
  ...
  _renderComingSoon() {
    ...
    if(displayComingSoon) {
      return(
        <TouchableWithoutFeedback onPress={this._on3DButtonTapped}>
          <View style={{flex: 1, position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, alignItems: 'center', justifyContent: 'center'}}>
            <Image
              source={require("../Assets/Slide05.jpg")}
              style={{
