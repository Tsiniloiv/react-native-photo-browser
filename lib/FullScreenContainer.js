...
export default class FullScreenContainer extends React.Component {
  ...
  _buildSwiper() {
    ...
    const builtList = mediaList.map(function(item) {
        if(item.type === 'photo') {
          return(<View key={data.indexOf(item)} style={styles.container}><TouchableHighlight onPress={this.logStuff}><Image style={styles.image}
                source={{uri: 'file://' + directory + item}}/></TouchableHighlight></View>);//
        } else if(item.type === 'video') {
          return(<View key={data.indexOf(item)} style={styles.container}><VideoPlayer ref={(ref: Video) => {this.videoRefs = {...this.videoRefs, ['REF-GALLERYVID' + item.index]: ref}}} 
                  source={vidSource}
                /></View>);//
        }
    });
