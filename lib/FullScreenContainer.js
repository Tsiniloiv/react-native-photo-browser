...
export default class FullScreenContainer extends React.Component {
    ...
    if(Platform.OS === 'android') {
      if(prevProps.mediaList.length != this.props.mediaList.length) {
        ...
        if(this.state.currentIndex < 0) {
          console.log("currentIndex is less than 0");
          console.log("Something got fucked somewhere.");
        }
        if(this.state.currentIndex > this.props.mediaList.length - 1) {
          console.log("currentIndex " + this.state.currentIndex + "greater than " + this.props.mediaList.length + "-1");
          console.log("Assume we just deleted media from the end of the list.")
          console.log("Thus, we should open page " + this.props.mediaList.length + "-1");
          this.openPage(this.props.mediaList.length - 1, false);
        } else {
          console.log("currentIndex " + this.state.currentIndex + " is not greater than " + this.props.mediaList.length + "-1");
          console.log("Assume we just deleted media from inside the list. No need to alter currentIndex, only refresh.");
          this.openPage(this.state.currentIndex, false);
        }
      }
  ...
  openPage(index, animated) {
    ...
    if (Platform.OS === 'ios') {
      ...
      console.log("Calling scrollBy to index " + index);
      this.swiperView.scrollBy(index, animated);
    }
  ...
  _onIndexChanged = (index) => {
    ...
    console.log(this.state.mediaList[index]);
  ...
  render() {
    ...
    //  return (<View style={styles.flex}><Text>gridView enabled</Text></View>);
    //} else if(!this.props.enableGrid) {
      return (
