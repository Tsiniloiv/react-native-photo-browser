...
export default class BottomBar extends React.Component {
  ...
  _render3DButton() {
    const {display3DButton, on3D } = this.props;

    if(display3DButton) {
      return(
        <TouchableOpacity style={[styles.button, {flex: 0, top:-5}]} onPress={on3D}>
          <MaterialCommunityIcons name="video-3d" size={35} color="white"/>
        </TouchableOpacity>
      );
    }
    return null;
