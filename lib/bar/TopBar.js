    ...
    if (displayGridButton) {
      return (
        <TouchableOpacity style={[styles.button, { flex: 0 }]} onPress={onGrid}>
          <Ionicons name="md-images" size={27} color="white" />
        </TouchableOpacity>
      );
    }
    return null;
    ...
    return (
      ...
      >
        <View style={styles.buttonContainer}>
          <View style={styles.buttonSubContainerLeft}>
            {this._renderOpenSideBar()}
          </View>
          <View style={styles.buttonSubContainerRight}>
...
const styles = StyleSheet.create({
  ...
  buttonSubContainerRight: {
    flex: 1,
    flexDirection: 'row',
    justifyContent: 'flex-end',
  },
  buttonSubContainerLeft: {
    flex: 1,
    flexDirection: 'row',
